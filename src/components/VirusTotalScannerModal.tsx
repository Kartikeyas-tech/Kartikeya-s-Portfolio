...
export default function VirusTotalScannerModal({ className = '' }: VirusTotalScannerProps) {
  ...
  return (
    <div className={`w-full bg-white relative border border-slate-200/90 rounded-2xl sm:rounded-3xl shadow-lg overflow-hidden text-slate-800 my-6 flex flex-col ${className}`}>
      {/* SECTION HEADER */}
      <div className="px-4 sm:px-6 py-4 sm:py-5 bg-slate-50/80 border-b border-slate-200 flex items-center justify-between shrink-0">
        <div className="flex items-center gap-3">
          ...
          <div>
            <div className="flex flex-wrap items-center gap-2">
              <h3 className="font-sans font-black text-base sm:text-xl text-slate-900 tracking-tight">
                File &amp; Threat Intelligence Inspector
              </h3>
              <span className="font-mono text-[9px] sm:text-[10px] font-bold px-2 py-0.5 rounded-full bg-emerald-100 text-emerald-800 border border-emerald-200 uppercase">
                PORTFOLIO SERVICE
              </span>
            </div>
        ...
        <div className="hidden sm:flex items-center gap-2 font-mono text-xs text-emerald-700 bg-emerald-50 px-3 py-1.5 rounded-xl border border-emerald-200 shrink-0">
      ...
      <div className="p-3.5 sm:p-6 space-y-5 sm:space-y-6">
            ...
            {/* SCAN MODE TABS */}
            <div className="flex items-center gap-1 bg-slate-100 p-1.5 sm:p-2 rounded-2xl border border-slate-200 w-full">
              <div className="grid grid-cols-3 gap-1 bg-white p-1 rounded-xl shadow-xs w-full">
                <button
                  onClick={() => { setActiveMode('file'); setScanResult(null); }}
                  className={`flex items-center justify-center gap-1.5 sm:gap-2 px-1 sm:px-3.5 py-2 rounded-lg font-mono text-[11px] sm:text-xs font-bold transition-all whitespace-nowrap ${
                    activeMode === 'file'
                ...
                >
                  ...
                  <span className="truncate">File Scan</span>
                </button>
                ...
                >
                  ...
                  <span className="truncate">URL Scan</span>
                </button>
                ...
                >
                  ...
                  <span className="truncate">Hash Lookup</span>
                </button>
              </div>
            </div>

            {/* INPUT SECTION BASED ON ACTIVE MODE */}
            <div className="bg-[#030912] border border-slate-800 p-3.5 sm:p-5 rounded-2xl">
              {activeMode === 'file' && (
